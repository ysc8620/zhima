...
class WeixinController extends Controller {
    ...
    function oauth(){
        ...
        if(isset($_GET['code']) ){
            ...
            header("location: ".U('/bao/weixin/userinfo').'&url='.$_GET['url']. '&token='.$data['access_token']);
            exit();
        }else{
            header("location: ".U('/bao/weixin/userinfo').'&url='.$_GET['url']);
            exit();
        }
    }
    ...
    public function pay(){
        ...
        if($id){
            ...
            if($order){
                ...
                if($hongbao['state'] != 1){
                    $this->error("红包不能支付", U('/bao/hongbao/detail', array('id'=>$hongbao['number_no'])));
                    exit(1);
                }

                if($hongbao['total_part'] <= $hongbao['total_num']){
                    $this->error("红包已经凑齐", U('/bao/hongbao/detail', array('id'=>$hongbao['number_no'])));
                    exit(1);
                }

                if(($hongbao['addtime'] + 86400) < time() ){
                    $this->error("红包已经过期", U('/bao/hongbao/detail', array('id'=>$hongbao['number_no'])));
                    exit(1);
                }
                ...
                if($order['state'] == 1){
                    ...
                    if($amount < 1){
                        $this->error("红包金额不对能支付", U('/bao/hongbao/detail', array('id'=>$order['number_no'])));
                        exit(1);
                    }
                    $data['body'] = "凑红包";
                    $data['attach'] = "凑红包";
                    ...
                    $this->error("红包状态不能支付", U('/bao/hongbao/detail', array('id'=>$order['number_no'])));
                    exit(1);
                }
            }

        }
        $this->error("红包状态不能支付", U('/bao/notes'));
        exit(1);
    }
